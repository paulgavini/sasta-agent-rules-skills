...
The agent must not use equation images as the default format for formulas or worked examples. Equations should be text-based, editable and accessible wherever possible.
...
Use an equation image only when the user explicitly wants a visual or the target platform cannot render the equation any other way. Even then, provide editable source text alongside it.
